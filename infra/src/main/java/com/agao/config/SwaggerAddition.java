...
public class SwaggerAddition implements ApiListingScannerPlugin {
    ...
    public List<ApiDescription> apply(DocumentationContext context) {
        ...
        ApiDescription loginApi = new ApiDescription(
                null,
                "/api/login",
                null,
                "登录",
                Lists.newArrayList(loginOperation),
                false);
        ...
        Operation logoutOperation = new OperationBuilder(new CachingOperationNameGenerator())
                ...
                .responses(Sets.newHashSet(new ResponseBuilder().code("200").description("OK").build()))
                .build();
        ApiDescription logoutApi = new ApiDescription(
                null,
                "/api/logout",
                null,
                "注销",
                Lists.newArrayList(logoutOperation),
                false);
