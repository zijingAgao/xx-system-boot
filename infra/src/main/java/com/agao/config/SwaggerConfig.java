...
public class SwaggerConfig {
    @Value("${xx-system.swagger.enable:false}")
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.OAS_30).apiInfo(apiInfo())
                ...
                // 扫描的路径包
                .apis(RequestHandlerSelectors.basePackage("com.agao"))
                // 指定路径处理PathSelectors.any()代表所有的路径
                .paths(PathSelectors.any()).build().pathMapping("/");
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("xx-system在线接口文档-swagger")
                .description("springboot2.7.15 | swagger3.0")
                // 作者信息
                .contact(new Contact("Agao", "xx-system.com", "agaozijing@gmail.com"))
                .version("1.0.0").build();
    }
